export class DataRefine {
  ...
  animate() {
    ...
    for(var i = this.string.length - 1; i >= 0; i --) {
      var y = this.startY + this.fontSize * i;
    	if (this.string[i].length == 0) { this.string.splice(i, 1); }
      for (var k = this.string[i].length - 1; k >= 0; k --) {

        this.checkPos(i, k);

        if ( y > this.offset - this.fontSize && y < this.stageHeight - this.offset ) {
  ...
  dataRefine(i, k) {
    ...
    if (this.isDown == 4) {
      ...
      else if (( k == 0 || k == this.string[i].length - 1 ) && this.string[i][k].str == ' ') {this.string[i].splice(k, 1); }
