...
class App {
  ...
  animate() {
    ...
    if (this.curItem) {
      ...
      this.ctx.beginPath();
      this.ctx.arc(this.curItem.centerPos.x, this.curItem.centerPos.y, 5, 0, Math.PI * 2);
      this.ctx.fill();
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(this.curItem.centerPos.x, this.curItem.centerPos.y);
  ...
  onDown(e) {
    ...
    for (let i = this.items.length - 1; i >=0; i--) {
      ...
      if (item) {
        this.curItem = item;
        const index = this.items.indexOf(item); // 이  부분이
        this.items.push(this.items.splice(index, 1)[0]); // jonna 이해  안  감
        break;
      }
    }
  }
