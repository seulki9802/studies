    ...
    for (var k = 0; k < this.ls[0].length; k ++) {
      ...
      for (var i = 0; i < this.ls.length; i ++) {
        ...
        console.log("!");
          ...
          for (var t = 0; t < 8; t ++) {
            var data = infos[n].children[t].textContent;
            if (data != undefined) { info[t] = data; }
            else { info[t] = "unknown"; }
            // info[i] = infos[n].children[i].textContent;
          }
          ...
          this.ls[i][k] = new Picture(
            x, y, this.width,
            info[0], // name
            info[1], info[2], info[3], //birth
            info[4], info[5], info[6], //death
            info[7], // image
            0,
            n
          );
          ...
          this.ls[i][k] = new Picture(x, y, this.width);
        }
        n += 1
      }
