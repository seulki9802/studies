import {Gallery} from './gallery3.js'
...
class App {
  ...
  animate() {
    window.requestAnimationFrame(this.animate.bind(this));
    this.moveX *= 0.99;
		if (this.on != 1) { this.ctx.clearRect(0, 0, this.stageWidth, this.stageHeight); }
		this.gallery.animate(this.ctx, this.moveX, this.on);
  }
  ...
  onUp(e) {
    ...
    if (this.onclick == e.clientX) {
      this.on += 1;
      this.moveX = 0;
      this.gallery.onClick(e.clientX, e.clientY, this.on);
    }
  }
