...
for (i = 0; i <= pageNumber; i ++) {
  pages[i] = {page: i, pageX: canvas.width * i}
}
...
for (i = 0; i < stone01_Number; i++) {
  // if ( i < 10 ) {
    var stone01_X = getRandomInt(canvas.width*0.5, canvas.width*10);
    var stone01_Y = getRandomInt(spearSize + stone01_Size, canvas.height - spearSize - stone01_Size);
    stone01_s[i] = { x : stone01_X, y : stone01_Y, size : stone01_Size};
  // }
  // else if ( i >= 10 && i < 20) {
  //   var stone01_X = getRandomInt(canvas.width*2, canvas.width*3);
  //   var stone01_Y = getRandomInt(spearSize + stone01_Size, canvas.height - spearSize - stone01_Size);
  //   stone01_s[i] = { x : stone01_X, y : stone01_Y, size : stone02_Size};
  // }
  // else {
  //   var stone01_X = getRandomInt(canvas.width*3, canvas.width*4);
  //   var stone01_Y = getRandomInt(spearSize + stone01_Size, canvas.height - spearSize - stone01_Size);
  //   stone01_s[i] = { x : stone01_X, y : stone01_Y, size : stone03_Size};
  // }
}
...
function drawRunningDistance() {
  ...
  ctx.restore();
  if (runningDistance == canvas.width * (2 - 1)) { stone01Speed += 2 }
  if (runningDistance == canvas.width * (3 - 1)) { stone01Speed += 3 }
}
...
function drawPage() {
  ...
  ctx.beginPath();
  for(i = 0; i<= pageNumber; i ++) {
    ctx.moveTo(pages[i].pageX, 0);
    ctx.lineTo(pages[i].pageX, canvas.height);
    ctx.font = "50px Arial";
    ctx.fillStyle = "red";
    ctx.textAlign = "center";
    ctx.fillText("page" + pages[i].page, pages[i].pageX, canvas.height/3);
    // pages[i].pageX -= stone01Speed;
    pages[i].pageX -= pageSpeed;
  }
  ctx.stroke();
  ctx.closePath();
  ctx.restore();
